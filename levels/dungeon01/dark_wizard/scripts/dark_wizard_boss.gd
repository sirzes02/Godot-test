...
const ENERGY_EXPLODE_SCENE: PackedScene = preload("res://levels/dungeon01/dark_wizard/energy_explosion.tscn")
...
@onready var position_target: Node2D = $PositionTarget
...
@onready var beam_attacks_node: Node2D = $BeamAttacks
...
func _ready() -> void:
	...
	for child in position_target.get_children():
		positions.append(child.global_position)
	position_target.visible = false
	
	for beam in beam_attacks_node.get_children():
...
func shoot_orb() -> void:
	var orb: Node2D = ENERGY_BALL_SCENE.instantiate()
...
func damage_taken(_hurt_box: HurtBox) -> void:
	if animation_player_damaged.current_animation == "damaged" or hurt_box.damage == 0:
		return
...
func explode(_p: Vector2 = Vector2.ZERO) -> void:
	var node: Node2D = ENERGY_EXPLODE_SCENE.instantiate()
	node.global_position = boss_node.global_position - _p
